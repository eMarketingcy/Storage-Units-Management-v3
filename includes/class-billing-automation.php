...
class SUM_Billing_Automation {
    ...
    public function update_rental_periods_after_payment($customer_id, $months_paid) {
        ...
        $customer = $this->customer_db->get_customer($customer_id);
        if (!$customer) {
            error_log("SUM Billing: Customer {$customer_id} not found for period extension");
            return false;
        }
        ...
        // Get all units for this customer
        $units = $wpdb->get_results($wpdb->prepare(
            "SELECT id, period_until, unit_name FROM {$units_table}
             WHERE customer_name = %s AND is_occupied = 1",
            $customer['full_name']
        ));

        // Get all pallets for this customer
        $pallets = $wpdb->get_results($wpdb->prepare(
            "SELECT id, period_until, pallet_name FROM {$pallets_table}
        ...
        error_log("SUM Billing: Period extension complete - {$units_updated} units and {$pallets_updated} pallets updated");
