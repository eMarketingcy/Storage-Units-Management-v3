...
class SUM_Customer_PDF_Generator {
            ...
            <table class="header-table">
                <tr>
                    ...
                    <td width="50%" style="text-align: right;">
                        <div class="invoice-title accent-color">INVOICE</div>
   ...
   /**
    ...
    private function generate_rental_item_row($rental, $currency_symbol) {
        // --- FIX: Directly read the month count from the billing result ---
        $months = isset($rental['billing_result']['occupied_months']) ? $rental['billing_result']['occupied_months'] : 1;
        
        $total = isset($rental['calculated_total']) ? $rental['calculated_total'] : $rental['monthly_price'];
        
        return '
            <tr>
                <td>' . ucfirst($rental['type']) . ' Storage - ' . esc_html($rental['name']) . '</td>
                <td style="text-align: center;">' . esc_html($months) . '</td>
